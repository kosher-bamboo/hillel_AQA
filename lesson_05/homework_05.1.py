...
founded_items = {}
for key in car_data:
    if (car_data.get(key)[1] >= search_criteria[0]  # compare year[1] >=
            and car_data.get(key)[2] >= search_criteria[1]  # compare engine_volume[2] >=
            and car_data.get(key)[4] <= search_criteria[2]):  # compare price[4] <=
        founded_items[key] = car_data.get(key)

sorted_founded_items = sorted(founded_items.items(), key=lambda x: x[1][4])  # sort by price [4]

index = 0
for key, value in sorted_founded_items:
    if index < 5:
        print(f"{key}: {value}")
        index += 1
